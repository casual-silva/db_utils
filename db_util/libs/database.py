...
def implode_condition(args, encape='`'):
    ...
    示例        [('id',1),'|',('status','<',1)] 其等价于 id = 1 or status < 1
                [('id',1),'|',[('status','<',1),('name','like','Silva')]] 其等价于 id = '1' or (status < '1' AND name like '%Silva%')
                [('id',1),'|',[('status','<',1),('name','like','Silva'),[('like','like','computer'),'|',('age','>',18)]]]
                其等价于 id = '1' or (status < '1' AND name like '%Silva%' AND (`like` like '%computer%' or `age` > 18))
...
class db_mysql:
    ...
    以下方法中codition为限制条件，可以为字典和列表
              ...
              data 为更新或添加的数据，可以为字典或者列表（列表必须为列表或元祖对，即[(key,val),...]）
                   键值对应为字段名
              example : insert('order',data = {'name':'Silva','create_time':121545015})
